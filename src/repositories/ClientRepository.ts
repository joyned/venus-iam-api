...
export class ClientRepository {
    ...
    async persist(client: Client) {
        ...
        if (!client.id) {
            ...
            result = await executeQuery(INSERT, [client.id, client.name, client.url, client.clientSecret, client.image]);
        } else {
            result = await executeQuery(UPDATE, [client.id, client.name, client.url, client.image]);
        }
